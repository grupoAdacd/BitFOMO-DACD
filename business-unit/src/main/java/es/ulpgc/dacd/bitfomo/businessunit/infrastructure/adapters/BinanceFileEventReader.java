...
public class BinanceFileEventReader implements EventReader {
    ...
    private void readFile(File file, EventProcessor processor) {
        try (BufferedReader reader = new BufferedReader(new FileReader(file))) {
            ...
            System.err.println("Error leyendo archivo de Binance: " + e.getMessage());
        }
    }
    ...
    private void processLine(String line, EventProcessor processor) {
        ...
        if (!cleanedLine.isEmpty()) {
            processor.processBinanceEvent(cleanedLine);
        }
    }
}